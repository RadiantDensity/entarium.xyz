    ...
    xhr.onload = async () => {
      ...
      if (xhr.status === 200) {
        try {
          ...
          // Fire-and-forget n8n webhook (optional, for Notion logging)
          fetch("https://radiantdensity.app.n8n.cloud/webhook/YOUR-WEBHOOK-ID", {
            method: "POST",
    ...
    <main className="flex flex-col items-center justify-center min-h-screen p-4">
      ...
      {cid && (
        <div className="mt-4 p-4 bg-green-100 text-green-900 rounded w-full max-w-sm text-center">
          ...
          <div className="break-all">CID: {cid}</div>
        </div>
      )}
      {error && (
